...
# Function to play sound with variable frequency based on distance
def play_sound_on_left(distance):
    playback_speed = 1.0 + (1.0 - min(distance / 2, 1.0))  # Adjust this formula as needed
    wave_obj = sa.WaveObject.from_wave_file("left.wav")
    play_obj = wave_obj.play(playback_speed=playback_speed)
    play_obj.wait_done()

def play_sound_on_right(distance):
    playback_speed = 1.0 + (1.0 - min(distance / 2, 1.0))  # Adjust this formula as needed
...
def run():
    ...
    while True:
        try:
            ...
            # Process and annotate both images and determine average depth for person detection
            avg_depth_left = avg_depth_right = 0
            person_count_left = person_count_right = 0
                ...
                for box, lbl in zip(bbox, label):
                    if lbl == 'person':
                        ...
                        if image is left_image:
                            ...
                            person_count_right += 1

            if person_count_left > 0:
                avg_depth_left /= person_count_left
                play_sound_on_left(avg_depth_left)

            if person_count_right > 0:
                avg_depth_right /= person_count_right
                play_sound_on_right(avg_depth_right)
