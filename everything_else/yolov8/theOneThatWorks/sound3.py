...
PERSON_WIDTH = 0.5  # Average width of a person in meters
CALIBRATED_FOCAL_LENGTH = 500  # Example focal length, needs calibration
STEREO_BASELINE = 0.3  # Distance between two cameras in meters, needs calibration

def calculate_distance(object_width_in_pixels, focal_length):
    return (PERSON_WIDTH * focal_length) / object_width_in_pixels
...
def run(camera_url, window_name, sound_function, is_left_camera):
    ...
    while True:
        ...
        for box, lbl in zip(bbox, label):
            ...
            distance = calculate_distance(object_width_in_pixels, CALIBRATED_FOCAL_LENGTH)
