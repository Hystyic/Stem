...
def process_frame(frame, model, classNames):
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            cls = int(box.cls[0])
            cv2.putText(frame, f"{classNames[cls]}: {math.ceil(box.conf[0]*100)/100}", (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        while True:
            if frame_buffer1 and frame_buffer2:
                future1 = executor.submit(process_frame, frame_buffer1[-1], model, classNames)
                future2 = executor.submit(process_frame, frame_buffer2[-1], model, classNames)
