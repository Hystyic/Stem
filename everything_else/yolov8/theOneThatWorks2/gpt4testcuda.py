...
# Function to detect objects using cvlib (CUDA accelerated if available)
def detect_objects_cuda(image):
    bbox, label, conf = cv.detect_common_objects(image, enable_gpu=True)
...
def run():
    ...
    while True:
        try:
            ...
            # Detect objects in both images using CUDA
            bbox_left, label_left, conf_left = detect_objects_cuda(left_image)
            bbox_right, label_right, conf_right = detect_objects_cuda(right_image)
